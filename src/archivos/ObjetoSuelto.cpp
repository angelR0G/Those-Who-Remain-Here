...
ObjetoSuelto::ObjetoSuelto(vector<float> p, Sprite* s, Textura* t, int id):ObjetoSala(p, s, t) {
    idObjeto    = id;

    FabricaObjetos* fabrica   = FabricaObjetos::Instancia();
...
//Destructor
ObjetoSuelto::~ObjetoSuelto() {}

//Elimina el material del mapa y lo anyade al inventario del jugador
void ObjetoSuelto::recoger(Inventario* inv) {
    if(!recogido) {
        recogido = true;

        Objeto* obj = FabricaObjetos::Instancia()->fabricarObjetoPorID(idObjeto);
        if(obj != nullptr) {
            inv->anyadeObjeto(obj);

            //Dependiendo del objeto, puede que aplique su efecto al recogerlo
            Jugador* jugador = nullptr;
            switch (idObjeto)
            {
            case 0:
                //Aumenta la salud maxima del jugador en 1
                jugador = EstadoPartida::Instancia()->getJugador();
                jugador->setVidaMax(jugador->getVidaMax() + 1);
                break;
            case 1:
                //Aumenta la salud maxima del jugador en 2
                jugador = EstadoPartida::Instancia()->getJugador();
                jugador->setVidaMax(jugador->getVidaMax() + 2);
                break;
            case 4:
                //Aumenta la velocidad de carga del poder
                jugador = EstadoPartida::Instancia()->getJugador();
                jugador->setCargaTiempoVel(jugador->getCargaTiempoVel() + 50);
                break;
            }
        }
    }
...
//Mostrar informacion de objeto
void ObjetoSuelto::mostrarInformacion(){
    ...
    descripcion   = to_string(fabrica->getDescripcion(idObjeto));
    */

    texto->setString("Objeto: "+nombre);
    textod->setString(descripcion);
    ...
    marcoN->setVertexPosition(0, (int)posicion[0] - anchotxt/2 - anchotxt/8, (int)posicion[1] - al_f*2          - texto->getAlto() - textod->getAlto());
    marcoN->setVertexPosition(1, (int)posicion[0] + anchotxt/2 + anchotxt/8, (int)posicion[1] - al_f*2          - texto->getAlto() - textod->getAlto());
    ...
    texto->setPosicion( posicion[0]-texto->getAncho()/2  ,posicion[1]-2*al_f-texto->getAlto()-textod->getAlto());
    textod->setPosicion(posicion[0]-textod->getAncho()/2 ,posicion[1]-al_f-texto->getAlto()-textod->getAlto());
    ...
    int marcoY = -(marcoN->getVertexPosition(1)[1]-marcoN->getVertexPosition(0)[1])   + posicion[1] - 2*al_f - texto->getAlto() - textod->getAlto();
