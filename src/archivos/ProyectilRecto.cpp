...
*/
ProyectilRecto::ProyectilRecto(int dJ, int dE, Sprite* s, vector<float> pos, vector<float> dir, int vel, int a, vector<int> runas) : Proyectil(dJ, dE, s, pos, dir, vel, a) {
    runasElementales = runas;
}

//Destructor
ProyectilRecto::~ProyectilRecto() {
    runasElementales.clear();
}

//Actualiza el proyectil
void ProyectilRecto::update(Tiempo elapsedTime) {
    bool colision = false;
...
bool ProyectilRecto::comprobarColisionMapa() {
    bool colision = false;
    EstadoPartida* estado = EstadoPartida::Instancia();
